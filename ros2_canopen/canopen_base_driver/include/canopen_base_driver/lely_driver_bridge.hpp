...
public:
  ...
  std::shared_ptr<PDOMap> createPDOMapping() {
    ...
    //     std::cout << "Found sub object: index=" << std::hex << (int)index
  ...
  bool checkObjRPDO(uint16_t idx, uint8_t subidx) {
    // std::cout << "Checking for rpo mapping of object: index=" << std::hex
    //           << (int)idx << " subindex=" << (int)subidx << std::endl;
    for (int i = 0; i < 256; i++) {
  ...
  bool checkObjInPDO(uint8_t pdo, uint16_t mapping_idx, uint16_t idx,
                     uint8_t subindex) {
    for (int i = 1; i < 9; i++) {
...
protected:
   ...
   * @param what
   */
  virtual void OnBoot(canopen::NmtState st, char es,
   ...
   * @todo This function should use a threadsafe queue not the icky
...
public:
  ...
  LelyDriverBridge(ev_exec_t *exec, canopen::AsyncMaster &master, uint8_t id,
                   std::string name, std::string eds, std::string bin,
                   std::chrono::milliseconds timeout = 20ms)
      : FiberDriver(exec, master, id), rpdo_queue(new SafeQueue<COData>()),
  ...
  template <typename T>
  std::future<bool> async_sdo_write_typed(uint16_t idx, uint8_t subidx,
                                          T value) {
    ...
    if (sub == nullptr) {
      std::cout << "async_sdo_write_typed: id=" << (unsigned int)this->get_id()
    ...
    this->SubmitWrite(
        idx, subidx, value,
        [this, value, prom](uint8_t id, uint16_t idx, uint8_t subidx,
                            ::std::error_code ec) mutable {
          if (ec) {
            prom->set_exception(lely::canopen::make_sdo_exception_ptr(
  ...
  template <typename T>
  bool sync_sdo_write_typed(uint16_t idx, uint8_t subidx, T value,
                            std::chrono::milliseconds timeout) {
    auto fut      = async_sdo_write_typed(idx, subidx, value);
    auto wait_res = fut.wait_for(timeout);
    if (wait_res == std::future_status::timeout) {
      std::cout << "sync_sdo_write_typed: id=" << (unsigned int)this->get_id()
                << " index=0x" << std::hex << (unsigned int)idx
                << " subindex=" << (unsigned int)subidx << " timed out."
                << std::endl;
      return false;
    }
    bool res = false;
  ...
  std::future<T> async_sdo_read_typed(uint16_t idx, uint8_t subidx) {
    ...
    lely::COSub *sub  = this->dictionary_->find(idx, subidx);
    if (sub == nullptr) {
      std::cout << "async_sdo_read: id=" << (unsigned int)this->get_id()
                << " index=0x" << std::hex << (unsigned int)idx
                << " subindex=" << (unsigned int)subidx
                << " object does not exist" << std::endl;
      try {
        throw lely::canopen::SdoError(this->get_id(), idx, subidx,
    ...
    this->SubmitRead<T>(
        idx, subidx,
        [this, prom](uint8_t id, uint16_t idx, uint8_t subidx,
  ...
  template <typename T>
  bool sync_sdo_read_typed(uint16_t idx, uint8_t subidx, T &value,
                           std::chrono::milliseconds timeout) {
    auto fut      = async_sdo_read_typed<T>(idx, subidx);
    auto wait_res = fut.wait_for(timeout);
    if (wait_res == std::future_status::timeout) {
      std::cout << "sync_sdo_read_typed: id=" << (unsigned int)this->get_id()
                << " index=0x" << std::hex << (unsigned int)idx
                << " subindex=" << (unsigned int)subidx << " timed out."
                << std::endl;
  ...
  bool wait_for_boot() {
    ...
    if ((boot_status != 0) && (boot_status != 'L')) {
      throw std::system_error(boot_status, LelyBridgeErrCategory(),
                              "Boot Issue");
    } else {
      booted.store(true);
      return true;
  ...
  template <typename T> void submit_write(COData data) {
    T value = 0;
    std::memcpy(&value, &data.data_, sizeof(value));

    this->SubmitWrite(
        data.index_, data.subindex_, value,
        [this, value](uint8_t id, uint16_t idx, uint8_t subidx,
                      ::std::error_code ec) mutable {
          if (ec) {
            this->sdo_write_data_promise->set_exception(
                lely::canopen::make_sdo_exception_ptr(id, idx, subidx, ec,
                                                      "AsyncDownload"));
  ...
  template <typename T> void submit_read(COData data) {
    this->SubmitRead<T>(
