...
template <>
void NodeCanopenBaseDriver<rclcpp_lifecycle::LifecycleNode>::configure(
    bool called_from_base) {
  try {
    this->non_transmit_timeout_ = std::chrono::milliseconds(
        this->config_["non_transmit_timeout"].as<int>());
  } catch (...) {
  }
  RCLCPP_INFO_STREAM(
      this->node_->get_logger(),
      "Non transmit timeout"
          << static_cast<int>(this->non_transmit_timeout_.count()) << "ms");
  ...
  if (diagnostic_enabled_.load()) {
    try {
      diagnostic_period_ms_ =
          this->config_["diagnostics"]["period"].as<std::uint32_t>();
    } catch (...) {
      RCLCPP_ERROR(
          this->node_->get_logger(),
    ...
    diagnostic_updater_ =
        std::make_shared<diagnostic_updater::Updater>(this->node_);
...
void NodeCanopenBaseDriver<NODETYPE>::activate(bool called_from_base) {
  nmt_state_publisher_thread_ = std::thread(
      std::bind(&NodeCanopenBaseDriver<NODETYPE>::nmt_listener, this));
  ...
  if (diagnostic_enabled_.load()) {
    ...
    diagnostic_updater_->add(
        "diagnostic updater", this,
        &NodeCanopenBaseDriver<NODETYPE>::diagnostic_callback);
  }
}
...
void NodeCanopenBaseDriver<NODETYPE>::add_to_master() {
  ...
  std::shared_ptr<std::promise<std::shared_ptr<ros2_canopen::LelyDriverBridge>>>
      prom;
  prom = std::make_shared<
      std::promise<std::shared_ptr<ros2_canopen::LelyDriverBridge>>>();
  std::future<std::shared_ptr<ros2_canopen::LelyDriverBridge>> f =
      prom->get_future();
  this->exec_->post([this, prom]() {
    ...
    this->lely_driver_ = std::make_shared<ros2_canopen::LelyDriverBridge>(
        *(this->exec_), *(this->master_), this->node_id_,
        this->node_->get_name(), this->eds_, this->bin_,
        std::chrono::milliseconds(this->sdo_timeout_ms_));
    this->driver_ = std::static_pointer_cast<lely::canopen::BasicDriver>(
        this->lely_driver_);
  ...
  this->driver_ =
      std::static_pointer_cast<lely::canopen::BasicDriver>(this->lely_driver_);
  ...
  if (diagnostic_enabled_.load()) {
    diagnostic_collector_->updateAll(diagnostic_msgs::msg::DiagnosticStatus::OK,
                                     "Device ready", "DEVICE",
                                     "Added to master.");
  }
}
...
void NodeCanopenBaseDriver<NODETYPE>::remove_from_master() {
  ...
  if (diagnostic_enabled_.load()) {
    diagnostic_collector_->updateAll(
        diagnostic_msgs::msg::DiagnosticStatus::ERROR, "Device removed",
        "DEVICE", "Removed from master.");
  }
}
template <class NODETYPE> void NodeCanopenBaseDriver<NODETYPE>::nmt_listener() {
  while (rclcpp::ok()) {
    ...
    while (f.wait_for(this->non_transmit_timeout_) !=
...
void NodeCanopenBaseDriver<NODETYPE>::on_emcy(COEmcy emcy) {
  diagnostic_collector_->summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR,
