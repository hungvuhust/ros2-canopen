...
bool Motor402::switchMode(uint16_t mode) {
  ...
  { // wait for switch
    ...
    if (monitor_mode_) {
      ...
      while (mode_id_ != mode && std::chrono::steady_clock::now() < abstime) {
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(20)); // wait some time
        lock.lock();
      }
    }
    ...
    if (mode_id_ == mode) {
      ...
      okay           = true;
      if (enable_diagnostics_.load()) {
        this->diag_collector_->addf("cia402_mode", "Mode switched to: %d",
                                    mode);
      }
    } else {
      RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"),
      ...
      if (enable_diagnostics_.load()) {
        this->diag_collector_->addf("cia402_mode", "Mode switch timed out: %d",
                                    mode);
      }
    }
  }
...
bool Motor402::switchState(const State402::InternalState &target) {
  ...
  while (state != target_state_) {
    ...
    if (!success) {
      ...
      this->diag_collector_->addf("cia402_state", "State switched to: %d",
                                  next);
    }
    lock.unlock();
    if (state != next && !state_handler_.waitForNewState(abstime, state)) {
      ...
      if (enable_diagnostics_.load()) {
        this->diag_collector_->addf("cia402_state",
                                    "State transition timed out: %d -> %d",
                                    state, next);
      }
      return false;
    }
...
bool Motor402::readState() {
  ...
  // RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Mode
  // %hhi",new_mode);
  ...
  if (new_mode != mode_id_) {
    RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"),
                "Read state mode id( 6061): 0x%x", new_mode);
    mode_id_ = new_mode;
  ...
  //   {
  //     RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Internal limit
  //     active");
  //   }
  //   else
  //   {
  ...
  //   }
  // }
  // RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Read state mode id(
  // 6061): 0x%x", new_mode);
  return true;
}
...
void Motor402::handleWrite() {
  ...
  if (start_fault_reset_.exchange(false)) {
    ...
    // RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver - khyem nguyen"),
    // "Control Word %d",control_word_);
...
void Motor402::handleDiag() {
  ...
  case State402::Ready_To_Switch_On:
    this->diag_collector_->summary(diagnostic_msgs::msg::DiagnosticStatus::OK,
                                   "Ready to switch on");
    break;
...
//   // RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Init: Switch
//   velocity");
...
//   RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Init: No Mode");
...
//   // RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"),
//   "OperationEnable(6040): %s", switchState(State402::Operation_Enable));
//   RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Control world(6040):
//   0x%x", control_word_);
//   RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Mode id(6060):
//   0x%x", Motor402::getMode());
...
bool Motor402::handleInit() {
  ...
  // }
  RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"),
              "Control world(6040): 0x%x", control_word_);
  RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Mode id(6060): 0x%x",
              Motor402::getMode());
