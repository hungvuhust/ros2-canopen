...
void NodeCanopen402Driver<rclcpp::Node>::init(bool called_from_base) {
  ...
  handle_init_service = this->node_->create_service<std_srvs::srv::Trigger>(
      ...
      std::bind(&NodeCanopen402Driver<rclcpp::Node>::handle_init, this,
  ...
  handle_set_mode_position_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          ...
          std::bind(
              &NodeCanopen402Driver<rclcpp::Node>::handle_set_mode_position,
  ...
  handle_set_mode_velocity_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          ...
          std::bind(
              &NodeCanopen402Driver<rclcpp::Node>::handle_set_mode_velocity,
              this, std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_cyclic_velocity_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          std::string(this->node_->get_name())
              .append("/cyclic_velocity_mode")
              .c_str(),
          std::bind(&NodeCanopen402Driver<
                        rclcpp::Node>::handle_set_mode_cyclic_velocity,
                    this, std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_cyclic_position_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          std::string(this->node_->get_name())
              .append("/cyclic_position_mode")
              .c_str(),
          std::bind(&NodeCanopen402Driver<
                        rclcpp::Node>::handle_set_mode_cyclic_position,
                    this, std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_interpolated_position_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          std::string(this->node_->get_name())
              .append("/interpolated_position_mode")
              .c_str(),
          std::bind(&NodeCanopen402Driver<
                        rclcpp::Node>::handle_set_mode_interpolated_position,
                    this, std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_torque_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          std::string(this->node_->get_name()).append("/torque_mode").c_str(),
          std::bind(&NodeCanopen402Driver<rclcpp::Node>::handle_set_mode_torque,
                    this, std::placeholders::_1, std::placeholders::_2));
  handle_set_target_service =
      this->node_->create_service<canopen_interfaces::srv::COTargetDouble>(
          ...
          std::bind(&NodeCanopen402Driver<rclcpp::Node>::handle_set_target,
                    this, std::placeholders::_1, std::placeholders::_2));
}

template <>
void NodeCanopen402Driver<rclcpp_lifecycle::LifecycleNode>::init(
  ...
  handle_init_service = this->node_->create_service<std_srvs::srv::Trigger>(
      std::string(this->node_->get_name()).append("/init").c_str(),
      std::bind(
          &NodeCanopen402Driver<rclcpp_lifecycle::LifecycleNode>::handle_init,
          this, std::placeholders::_1, std::placeholders::_2));

  handle_halt_service = this->node_->create_service<std_srvs::srv::Trigger>(
      std::string(this->node_->get_name()).append("/halt").c_str(),
      std::bind(
          &NodeCanopen402Driver<rclcpp_lifecycle::LifecycleNode>::handle_halt,
          this, std::placeholders::_1, std::placeholders::_2));

  handle_recover_service = this->node_->create_service<std_srvs::srv::Trigger>(
      std::string(this->node_->get_name()).append("/recover").c_str(),
      std::bind(&NodeCanopen402Driver<
                    rclcpp_lifecycle::LifecycleNode>::handle_recover,
                this, std::placeholders::_1, std::placeholders::_2));
      ...
      this->node_->create_service<std_srvs::srv::Trigger>(
          ...
          std::bind(
              &NodeCanopen402Driver<
                  rclcpp_lifecycle::LifecycleNode>::handle_set_mode_position,
              this, std::placeholders::_1, std::placeholders::_2));
  ...
  handle_set_mode_velocity_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          ...
          std::bind(
              &NodeCanopen402Driver<
                  rclcpp_lifecycle::LifecycleNode>::handle_set_mode_velocity,
              this, std::placeholders::_1, std::placeholders::_2));
  ...
  handle_set_mode_cyclic_velocity_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          std::string(this->node_->get_name())
              .append("/cyclic_velocity_mode")
              .c_str(),
          std::bind(&NodeCanopen402Driver<rclcpp_lifecycle::LifecycleNode>::
  ...
  handle_set_mode_torque_service =
      this->node_->create_service<std_srvs::srv::Trigger>(
          ...
          std::bind(
              &NodeCanopen402Driver<
                  rclcpp_lifecycle::LifecycleNode>::handle_set_mode_torque,
              this, std::placeholders::_1, std::placeholders::_2));
  ...
  handle_set_target_service =
      this->node_->create_service<canopen_interfaces::srv::COTargetDouble>(
          std::string(this->node_->get_name()).append("/target").c_str(),
          std::bind(&NodeCanopen402Driver<
                        rclcpp_lifecycle::LifecycleNode>::handle_set_target,
                    this, std::placeholders::_1, std::placeholders::_2));
}

template <>
void NodeCanopen402Driver<rclcpp_lifecycle::LifecycleNode>::configure(
    bool called_from_base) {
...
void NodeCanopen402Driver<NODETYPE>::activate(bool called_from_base) {
  ...
  motor_->set_diagnostic_status_msgs(this->diagnostic_collector_,
...
template <class NODETYPE> void NodeCanopen402Driver<NODETYPE>::publish() {
...
template <class NODETYPE> void NodeCanopen402Driver<NODETYPE>::add_to_master() {
...
void NodeCanopen402Driver<NODETYPE>::handle_set_target(
  ...
  if (this->activated_.load()) {
    ...
    } else if ((mode == MotorBase::Velocity) or
...
template <class NODETYPE> bool NodeCanopen402Driver<NODETYPE>::init_motor() {
  if (this->activated_.load()) {
    bool temp = motor_->handleInit();
    return temp;
  } else {
...
template <class NODETYPE> bool NodeCanopen402Driver<NODETYPE>::recover_motor() {
  if (this->activated_.load()) {
...
template <class NODETYPE> bool NodeCanopen402Driver<NODETYPE>::halt_motor() {
  if (this->activated_.load()) {
...
bool NodeCanopen402Driver<NODETYPE>::set_mode_position() {
  std::cout << "" << std::endl;
  std::cout << "[---------------set_mode_position-------------khyemnguyen------"
               "-------]"
            << std::endl;
  if (this->activated_.load()) {
    if (motor_->getMode() != MotorBase::Profiled_Position) {
      // RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Mode id: 0x%x",
      // motor_->getMode());
      return motor_->enterModeAndWait(MotorBase::Profiled_Position);
    } else {
      // RCLCPP_INFO(rclcpp::get_logger("canopen_402_driver"), "Mode id: 0x%x",
      // motor_->getMode());
      RCLCPP_INFO(rclcpp::get_logger("khyemnguyen"),
                  "Already in position mode: 0x%x", motor_->getMode());
      return true;
    }
  } else {
    return false;
...
bool NodeCanopen402Driver<NODETYPE>::set_mode_velocity() {
  std::cout << "" << std::endl;
  std::cout << "[---------------set_mode_velocity-------------khyemnguyen------"
               "-------]"
            << std::endl;
  if (this->activated_.load()) {
    if (motor_->getMode() != MotorBase::Profiled_Velocity) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("khyemnguyen"),
                  "Already in velocity mode: 0x%x", motor_->getMode());
      return true;
    }
  } else {
    return false;
...
bool NodeCanopen402Driver<NODETYPE>::set_target(double target) {
  if (this->activated_.load()) {
    ...
    // RCLCPP_INFO(this->node_->get_logger(), "Scaled target %f",
    // scaled_target);
...
void NodeCanopen402Driver<NODETYPE>::diagnostic_callback(
  ...
  stat.summary(this->diagnostic_collector_->getLevel(),
               this->diagnostic_collector_->getMessage());
  ...
  stat.add("cia402_state",
           this->diagnostic_collector_->getValue("cia402_state"));
}
